...
class BaseEnv(gym.Env):
    def __init__(
        ...
        #### Control parameters ####################################
        self.RATE_SCALE = 0.5*np.pi*np.array([1.0, 1.0, 1.0])
        self.I_GAIN = np.array([4, 4, 2]) * 1e-6
        self.P_GAIN = np.array([4, 4, 2]) * 1e-4
        ...
        self.observation_space = Dict(
            {
                "img": Box(
                    low=0, high=1, shape=(1, self.IMG_RES, self.IMG_RES), dtype=np.float32
                ),
                "last_3_actions": Box(
                    low=-1, high=1, shape=(3, 4), dtype=np.float32
                ),
                "kinematics": Box(
    ...
    def _getImages(self):
        ...
        self.DRONE_CAM_VIEW = p.computeViewMatrix(cameraEyePosition=cameraPos,
        ...
        w, h, rgb, dep, seg = p.getCameraImage(width=self.IMG_RES,
                                                 height=self.IMG_RES,
                                                 shadow=0,
                                                 viewMatrix=self.DRONE_CAM_VIEW,
                                                 projectionMatrix=DRONE_CAM_PRO,
                                                 flags=p.ER_SEGMENTATION_MASK_OBJECT_AND_LINKINDEX,
                                                 physicsClientId=self.CLIENT
                                                 )
        # w, h = self.IMG_RES, self.IMG_RES
        # rgb = np.zeros((self.IMG_RES, self.IMG_RES, 4))
        # dep = np.zeros((self.IMG_RES, self.IMG_RES))
        # seg = np.zeros((self.IMG_RES, self.IMG_RES))
        # stop_time = time.time()

        # print("Time taken to get images: ", (stop_time-start_time) * 1000, "ms")
    ...
    def _PyBulletDynamics(self, rpm):
        ...
        drag = drag_factors * np.dot(base_rot.T, np.array(self.vel))
    ...
    def _rateController(self, action):
        ...
        torque = self.P_GAIN * rate_error + self.I_GAIN * self.rate_error_sum
        ...
        MixerMatrix = np.array([
            ...
            np.array([-1,  1, -1,  1], dtype=np.float32) * self.KF/self.KM     # Yaw torque
        ])/4
