...
class DroneRenderer:
    def __init__(self, width=800, height=600):
        ...
        self.window = glfw.create_window(
            width, height, "Drone Axis", None, None)
        if not self.window:
            glfw.terminate()
        ...
        for i in np.arange(-grid_size, grid_size + grid_step, grid_step):
            ...
            # Lines along Y-axis
            grid_vertices.extend([-grid_size, i, -2] + grid_color)
            grid_vertices.extend([grid_size, i, -2] + grid_color)
    ...
    def _scroll_callback(self, window, xoffset, yoffset):
        ...
        self.camera.radius = np.clip(self.camera.radius, 2.0, 200.0)
    ...
    def render(self, position, rotation_matrix):
        self.update_fps()
...
if __name__ == "__main__":
    ...
    while not renderer.should_close():
        ...
        renderer.render(position, rotation)
